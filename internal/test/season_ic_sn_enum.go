...
func (v Season_Ic_Sn) String() string {
	return v.toString(season_ic_snEnumStrings, season_ic_snEnumIndex[:])
}

func (v Season_Ic_Sn) toString(concats string, indexes []uint16) string {
	o := v.Ordinal()
...
func Season_Ic_SnOf(v int) Season_Ic_Sn {
	...
	// an invalid result
	return Spring_Ic_Sn + Summer_Ic_Sn + Autumn_Ic_Sn + Winter_Ic_Sn + 1
}
...
// a number. The input representation is determined by None. It is used by AsSeason_Ic_Sn.
...
// a number. The input representation is determined by season_ic_snMarshalTextRep. It wraps Parse.
// The input case does not matter.
