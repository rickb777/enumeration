...
// AllSeason_Ic_TnEnums lists all 4 values in order.
var AllSeason_Ic_TnEnums = enum.IntEnums{
	Spring_Ic_Tn, Summer_Ic_Tn, Autumn_Ic_Tn, Winter_Ic_Tn,
}
...
func (v Season_Ic_Tn) String() string {
	return v.toString(season_ic_tnEnumStrings, season_ic_tnEnumIndex[:])
}

func (v Season_Ic_Tn) toString(concats string, indexes []uint16) string {
	o := v.Ordinal()
...
func Season_Ic_TnOf(v int) Season_Ic_Tn {
	...
	// an invalid result
	return Spring_Ic_Tn + Summer_Ic_Tn + Autumn_Ic_Tn + Winter_Ic_Tn + 1
}
...
func (v Season_Ic_Tn) MarshalText() (text []byte, err error) {
	if !v.IsValid() {
		return v.marshalNumberOrError()
	}

	return season_ic_tnMarshalNumber(v)
}

// season_ic_tnMarshalNumber handles marshaling where a number is required or where
// the value is out of range but season_ic_tnMarshalTextRep != enum.Ordinal.
// This function can be replaced with any bespoke function than matches signature.
var season_ic_tnMarshalNumber = func(v Season_Ic_Tn) (text []byte, err error) {
