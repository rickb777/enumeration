...
func (v Season_Nc_Ti) String() string {
	return v.toString(season_nc_tiEnumStrings, season_nc_tiEnumIndex[:])
}

func (v Season_Nc_Ti) toString(concats string, indexes []uint16) string {
	o := v.Ordinal()
...
func Season_Nc_TiOf(v int) Season_Nc_Ti {
	...
	// an invalid result
	return Spring_Nc_Ti + Summer_Nc_Ti + Autumn_Nc_Ti + Winter_Nc_Ti + 1
}
...
func (v Season_Nc_Ti) MarshalText() (text []byte, err error) {
	if !v.IsValid() {
