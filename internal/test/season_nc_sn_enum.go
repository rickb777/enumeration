...
func (v Season_Nc_Sn) String() string {
	return v.toString(season_nc_snEnumStrings, season_nc_snEnumIndex[:])
}

func (v Season_Nc_Sn) toString(concats string, indexes []uint16) string {
	o := v.Ordinal()
...
func Season_Nc_SnOf(v int) Season_Nc_Sn {
	...
	// an invalid result
	return Spring_Nc_Sn + Summer_Nc_Sn + Autumn_Nc_Sn + Winter_Nc_Sn + 1
}
...
// a number. The input representation is determined by None. It is used by AsSeason_Nc_Sn.
//
// Usage Example
//
//	v := new(Season_Nc_Sn)
...
// a number. The input representation is determined by season_nc_snMarshalTextRep. It wraps Parse.
