...
func (v Season_Uc_Sn) String() string {
	return v.toString(season_uc_snEnumStrings, season_uc_snEnumIndex[:])
}

func (v Season_Uc_Sn) toString(concats string, indexes []uint16) string {
	o := v.Ordinal()
...
func Season_Uc_SnOf(v int) Season_Uc_Sn {
	...
	// an invalid result
	return Spring_Uc_Sn + Summer_Uc_Sn + Autumn_Uc_Sn + Winter_Uc_Sn + 1
}
...
// a number. The input representation is determined by None. It is used by AsSeason_Uc_Sn.
//
// Usage Example
//
//	v := new(Season_Uc_Sn)
...
// a number. The input representation is determined by season_uc_snMarshalTextRep. It wraps Parse.
