...
func (v Season_Uc_Si) String() string {
	return v.toString(season_uc_siEnumStrings, season_uc_siEnumIndex[:])
}

func (v Season_Uc_Si) toString(concats string, indexes []uint16) string {
	o := v.Ordinal()
...
func Season_Uc_SiOf(v int) Season_Uc_Si {
	...
	// an invalid result
	return Spring_Uc_Si + Summer_Uc_Si + Autumn_Uc_Si + Winter_Uc_Si + 1
}
...
// a number. The input representation is determined by None. It is used by AsSeason_Uc_Si.
//
// Usage Example
//
//	v := new(Season_Uc_Si)
...
// a number. The input representation is determined by season_uc_siMarshalTextRep. It wraps Parse.
